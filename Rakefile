...
drafts_dir		= "_drafts"	  # directory for blog drafts
themes_dir      = ".themes"   # directory for blog files
new_post_ext    = "markdown"  # default new post file extension when using the new_post task
new_draft_ext	= "markdown"  # default new draft file extension when using the new_draft task
...
# usage rake new_link[my-new-link] or rake new_link['my new link'] or rake new_link (defaults to "new-link")
desc "Begin a new link post in #{source_dir}/#{posts_dir}"
task :new_link, :title do |t, args|
  if args.title
    title = args.title
  else
    title = get_stdin("Enter a title for your link post: ")
  end
  raise "### You haven't set anything up yet. First run `rake install` to set up an Octopress theme." unless File.directory?(source_dir)
  mkdir_p "#{source_dir}/#{posts_dir}"
  filename = "#{source_dir}/#{posts_dir}/#{Time.now.strftime('%Y-%m-%d')}-#{title.to_url}.#{new_post_ext}"
...
task :new_draft, :title do |t, args|
  ...
  filename = "#{source_dir}/#{drafts_dir}/#{Time.now.strftime('%Y-%m-%d')}-#{title.to_url}.#{new_draft_ext}"
  if File.exist?(filename)
    abort("rake aborted!") if ask("#{filename} already exists. Do you want to overwrite?", ['y', 'n']) == 'n'
  end
...
task :gen_preview => [:generate, :preview] do
end
